...
QHash<QString, double> ByBitParser::parseInstrumentTickSizes(const QByteArray& rawData)
{
	...
	try
	{
		...
		for (simdjson::dom::element item : array_list)
		{
			...
			std::string_view tickStr = item["priceFilter"]["tickSize"].get_string().value();
			double tickSize = fastParseDouble(tickStr);
			result.insert(QString::fromUtf8(sym.data(), static_cast<int>(sym.size())), tickSize);
		}
	}
	catch (const simdjson::simdjson_error& er)
	{
		qDebug() << "[BybitParser] parseInstrumentTickSizes error: " << er.what();
	}
	return result;
}
...
std::optional<OrderBookSnapshot> ByBitParser::parseOrderBook(const QByteArray& jsonMessage, QString& outSymbol, bool& outIsDelta)
{
	...
	try
	{
		...
		int64_t ts = root["ts"].get_int64().value();
