...
void BybitConnector::fetchAvailableSymbols()
{
	struct SyncContext
	{
		QList<std::pair<QString, QString>> accumulateSymbols;//symbol MarketType
		QHash<QString, double> accumulateTickSizes; //symbol и шаг цены
	...
	std::function<void(const QString&)> launchRequest = [this, syncCtx](const QString& category) 
	{
		...
		QObject::connect(reply, &QNetworkReply::finished, this, [self, reply, syncCtx, category]()
			...
			if (reply->error() == QNetworkReply::NoError)
			{
				...
				QStringList parsed = ByBitParser::parseAvailableSymbols(raw);
			...
			if (syncCtx->pendingRequests == 0)
			{
				...
				emit EventBus::instance().instrumentTickSizesLoaded("Bybit", syncCtx->accumulateTickSizes);
			}
			});
	};
...
void BybitConnector::onWsMessageReceived(const QString& message)
{
	...
	if (message.contains("\"orderbook."))
	{
		QString symbol;
		bool    isDelta = false;
		std::optional<OrderBookSnapshot> snap = ByBitParser::parseOrderBook(raw, symbol, isDelta);
		if (snap.has_value())
		{
